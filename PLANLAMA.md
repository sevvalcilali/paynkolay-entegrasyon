...
| 2026-09-11 | **v8 = v1 içeriği + sol menüsü, Gravür temasında (Şevval: "v1'in menüsünü ve içeriğini v8 temasına uydur"):** 69 alt sayfa v1'den birebir üretildi (menü, içerik ve Bu Sayfada listesi v1 ile otomatik denetimde aynı; farklar yalnızca v8 üst barı, içerik kimliği, Önceki/Sonraki gravür ayracı ve rozet). Kabuk: istenince açılan çekmece yerine **kalıcı sol menü** (≥1024px), dar ekranda soldan açılan çekmece; kurumsal üst menü kaldırıldı, üst bar v1 gibi (logo + DOCS, arama, tema, Panel Giriş, Başvurun). v8 CSS tek içerik işaretlemesine geçti: `ptable→dtable`, `copychip→copybtn(--dark)`, `btn--blue→btn--primary`; ölü kurallar silindi (kurumsal menü, çekmece başlığı, "BU BÖLÜMDE" kartı, itme efekti); eksik bileşenler Gravür dilinde eklendi (dil kutusu, GitBook rozeti, belge kontrol listesi, ekran görüntüleri, select alanları). Ana sayfa v8 gövdesini korudu; bölüm haritası v1 menü ağacıyla eşitlendi (4 API alt sayfası, Masterpass bölümü → 9 bölüm, iFrame ve iki Postman koleksiyonu). Tarayıcıda doğrulanan iki mobil hata düzeltildi: 390px'te üst bar taşıyordu (tema anahtarı sağ alta, Panel Giriş ≤480px'te gizli) ve üst barın backdrop-filter'ı sabit tema anahtarını bara hapsediyordu. v1 ve v8'de 14 aktif menü öğesine eksik `aria-current` eklendi, Ek Bilgiler ana sayfasının aktif işareti düzeltildi. v8: 70 sayfa, arama indeksi 305 kayıt, 0 kırık link, tanımsız sınıf yok. |
| 2026-09-11 | **v8 sol menü aç/kapa + üst bar hizası (Şevval: "soldaki menü açılır kapanır olsun; arama ve o satırdakiler konum bakımından güzel dursun"):** Masaüstünde üst bardaki menü düğmesi sol menüyü daraltıp açıyor (kolon yumuşakça kapanır, içerik tam genişliğe yayılır); tercih `localStorage`'da saklanıp head'deki `menustate.js` ile ilk boyamadan önce uygulanıyor — sayfa geçişlerinde menü bir an açılıp kapanmıyor. Mobilde aynı düğme çekmeceyi açar. Üst bar üç bölgeli ızgaraya geçti: marka bölgesi sol menü genişliğinde (ayırıcı çizgisi menü kenarıyla hizalı), arama orta bölgenin tam merkezinde 520px'e kadar, tema anahtarı ve düğmeler sağda aynı 36px yükseklikte. Tarayıcı ölçümüyle bulunan mobil düzeltmeler: ana sayfa kapsayıcısı ızgarada auto-margin yüzünden 591px'e şişiyordu (`width:100%`, aynı düzeltme `.content`'e — kısa sayfalar da tam genişlik), hero başlığı ekran genişliğine göre ölçekleniyor, destek bandındaki e-posta düğmesi dar ekranda kırılıyor, arama gizlenince eylemler sağda kalıyor, 360px altında DOCS rozeti gizleniyor. 320/360/390px ve masaüstünde yatay taşma yok. |
| 2026-09-11 | **3DS alt başlıkları Non-3DS ile aynı ada geçti (Şevval: "oluşturma ve sorgulama olacak, önceki/sonraki kutuları da öyle"):** `api-3ds-baslatma` → `api-3ds-odeme-olusturma` (Ödeme Oluşturma), `api-3ds-tamamlama` → `api-3ds-odeme-sorgulama` (Ödeme Sorgulama). İçerik adlara oturtuldu: sonuç dönüşü + CompletePayment (2 aşamalı) Ödeme Oluşturma'ya taşındı; Ödeme Sorgulama 3DS bağlamlı PaymentList sayfası oldu (ne zaman gerekir, IS_3D, STATUS=NEW). Pager zinciri: 3DS → Ödeme Oluşturma → Ödeme Sorgulama → Form ile Ödeme. Sekme başlıkları "(3DS)" / "(Non-3DS)" ile ayrıştı; düzyazıdaki kaçış karakterleri (\r, \n, \") metin olarak düzeltildi. v8, yeni `tools/sync-v8.py` ile v1'den yeniden üretildi; arama indeksleri yenilendi. QA: 0 kırık link, v1 = v8 içerik/menü/TOC. |
| 2026-09-11 | **Önceki/Sonraki denetimi + Ek Bilgiler Test Kartları (Şevval: "ek bilgilerde geçişlerde tahsilat metodu yazıyor, sıralamayı düzelt, genel pre/next kontrolü yap"):** Kök neden: Ek Bilgiler'deki Test Kartları, Ön Hazırlık sayfasına açılan kısayoldu — menü Ön Hazırlık'a atlıyor, kutu "Sonraki: Tahsilat Metodu" diyordu. Şevval'in seçimiyle Ek Bilgiler'e kendi `ek-bilgiler/test-kartlari.html` sayfası eklendi (içerik Ön Hazırlık sayfasından `tools/mirror-pages.py` ile üretilir). Zincir: Ek Bilgiler → Test Kartları → Hash Aracı → Hata Kodları → Banka Kodları → TLS Hataları → iFrame Kullanımı → SSS. Yeni `tools/check-pager.py` tüm sayfaların kutularını menü sırasına göre denetler (`--fix` ile düzeltir); v1 ve v8'de 0 sorun. v8 ana sayfa kutusu "SONRAKİ" → "Sonraki". Sayfa sayısı 71. |
| 2026-09-15 | **v1 Tahsilat Metodu başlık + dosya adları (Şevval: "Non-3DS/3DS alt başlıkları taksit bilgisi size ait / Paynkolay'a ait; Form ile Ödeme → Güvenli Ödeme; dosya adları da başlıklara uysun"):** Menü, h1, sekme başlığı, kartlar, Önceki/Sonraki ve arama indeksinde yeni adlar. Dosyalar: `form-ile-odeme` → `guvenli-odeme`, `api-(non)3ds-odeme-olusturma` → `api-(non)3ds-taksit-bilgisi-size-ait`, `api-(non)3ds-odeme-sorgulama` → `api-(non)3ds-taksit-bilgisi-paynkolaya-ait`; v1'deki tüm atıflar güncellendi. Sayfa içerikleri (düzyazı) değişmedi — Şevval verecek. v8'e henüz uygulanmadı. QA: 5874 link 0 kırık, check-pager 0 sorun, arama indeksi yeniden üretildi (313 kayıt). |
